...
def main():
    ...
    for coef in tqdm(np.linspace(1, 30, 30)):
        ...
        table_poisson.append(
            [lambdas["poisson"][-1], P1s["poisson"][-1], P2s["poisson"][-1], Ps_alg["poisson"][-1], Ps_imp["poisson"][-1]])
        table_cor_02.append([lambdas["cor02"][-1], P1s["cor02"][-1], P2s["cor02"][-1], Ps_alg["cor02"][-1], Ps_imp["cor02"][-1]])
        table_cor_04.append([lambdas["cor04"][-1], P1s["cor04"][-1], P2s["cor04"][-1], Ps_alg["cor04"][-1], Ps_imp["cor04"][-1]])
    ...
    plt.ylabel(r'$L$')
    plt.xlabel(r'$\lambda$')
    plt.legend((r'$c_{cor}^{(1)}$ =' + str(abs(round(qs1.queries_stream.c_cor[0], 3))) + r', $c_{cor}^{(2)}$=' + str(abs(round(qs1.queries_stream.c_cor[1], 3))),
                r'$c_{cor}^{(1)}$ =' + str(round(qs2.queries_stream.c_cor[0], 3)) + r', $c_{cor}^{(2)}$=' + str(round(qs2.queries_stream.c_cor[1], 3)),
                r'$c_{cor}^{(1)}$ =' + str(round(qs3.queries_stream.c_cor[0], 3)) + r', $c_{cor}^{(2)}$=' + str(round(qs3.queries_stream.c_cor[1], 3))
                ),
               loc=0)
    if PRINT_TITLE:
    ...
    plt.legend((r'$c_{cor}^{(1)}$ =' + str(abs(round(qs1.queries_stream.c_cor[0], 3))) + r', $c_{cor}^{(2)}$=' + str(
        abs(round(qs1.queries_stream.c_cor[1], 3))),
               ...
               loc=0)
    if PRINT_TITLE:
        plt.title('Зависимость $bar{w}_1$ от $lambda$ при различных \n коэффициентах корреляции длин двух соседних интервалов')
    ...
    plt.legend((r'$c_{cor}^{(1)}$ =' + str(abs(round(qs1.queries_stream.c_cor[0], 3))) + r', $c_{cor}^{(2)}$=' + str(
        abs(round(qs1.queries_stream.c_cor[1], 3))),
               ...
               loc=0)
    if PRINT_TITLE:
        plt.title('Зависимость $bar{w}_2$ от $lambda$ при различных \n коэффициентах корреляции длин двух соседних интервалов')
    ...
    np.savetxt(str.format("tables/BMMAP5_P_losses_lambda_cor_poisson_{}.csv", nowdate), table_poisson,
               header=r'$\lambda$, $P_{loss}^{(1)}$, $P_{loss}^{(2)}$, $P_{loss}, P_{loss}^{(imp)}$', delimiter=',', fmt='%.5f', comments='')
