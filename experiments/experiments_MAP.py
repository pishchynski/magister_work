...
import experiments_data.MAP_02_PH_PH as cor02
import experiments_data.MAP_04_PH_PH as cor04
import experiments_data.MAP_Poisson_NoRemoval as poisson_noremoval
import experiments_data.MAP_Poisson_PH_PH as poisson
import experiments_data.MAP_Poisson_PH_Poisson_NoRemoval as poisson_PH_poisson_noremoval
import two_priorities_qs as qs


def main():
    test_data_Poisson_initial = poisson.MapPoissonPhPh()
    test_data_Poisson = poisson.MapPoissonPhPh()

    test_data_02_initial = cor02.Map02PhPh()
    test_data_02 = cor02.Map02PhPh()

    test_data_04_initial = cor04.Map04PhPh()
    test_data_04 = cor04.Map04PhPh()
    
    test_data_Poisson_PH_poisson_noRemoval_initial = poisson_PH_poisson_noremoval.MapPoissonPhPoissonNoRemoval()
    test_data_Poisson_PH_poisson_noRemoval = poisson_PH_poisson_noremoval.MapPoissonPhPoissonNoRemoval()

    test_data_Poisson_noRemoval_initial = poisson_noremoval.MapPoissonNoRemoval()
    test_data_Poisson_noRemoval = poisson_noremoval.MapPoissonNoRemoval()
    ...
    for coef in np.linspace(0.01, 1, 100):
    ...
    plt.savefig(str.format("plots/MAP_L_lambda_cor_0_02_04_NoRemoval_{}.png", datetime.datetime.now()),
                bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(str.format("plots/MAP_P_loss_lambda_cor_0_02_04_NoRemoval_{}.png", datetime.datetime.now()),
                bbox_inches='tight')
    plt.close()
